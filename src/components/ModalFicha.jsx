    ...
    <div className="fixed inset-0 bg-gradient-to-br from-slate-900/70 via-slate-800/60 to-slate-900/70 backdrop-blur-md flex items-center justify-center p-4 z-[100] animate-[fadeIn_0.3s_ease-out]">
      ...
      <div className="bg-white/95 backdrop-blur-xl p-6 sm:p-8 rounded-[2.5rem] shadow-[0_25px_80px_rgba(0,0,0,0.4)] w-full max-w-md border border-white/50 relative animate-[fadeIn_0.5s_ease-out] overflow-hidden">
        ...
        <div className="flex justify-between items-start mb-6">
          ...
          <button 
            onClick={onClose} 
            className="w-10 h-10 bg-slate-100 hover:bg-rose-100 text-slate-400 hover:text-rose-600 rounded-full flex items-center justify-center transition-all duration-300 hover:rotate-90 hover:scale-110 group"
          >
        ...
        {/* 🚀 ACCIÓN ADMINISTRATIVA: Forzar Liberación por Urgencia */}
        <button 
          onClick={() => {
            const confirmar = window.confirm(`⚠️ ADVERTENCIA ADMINISTRATIVA:\n\n¿Estás seguro de liberar por la fuerza el cajón ${lugar.codigo}? Esta acción cerrará el ticket actual del usuario.`);
            if (confirmar) onLiberar(lugar.codigo);
          }}
          className="w-full bg-gradient-to-r from-rose-600 to-red-600 hover:from-rose-700 hover:to-red-700 text-white font-black py-4 rounded-2xl shadow-[0_10px_30px_rgba(225,29,72,0.3)] hover:shadow-[0_15px_40px_rgba(225,29,72,0.4)] transition-all duration-300 active:scale-95 text-base group relative overflow-hidden"
        >
